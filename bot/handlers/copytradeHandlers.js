...
const { getEthBalance, decryptData } = require('../baseBot');
const { CONFIG } = require('../../config');
const copyTradeService = require('../../services/copytrade');
const { CopyTradeOps, UserOps } = require('../../database/operations');
    ...
    try {
      switch (copyTradeState.state) {
        ...
        case COPYTRADE_STATES.AWAITING_PIN:
          ...
          try {
            await copyTradeService.addCopyTrade(
              userId,
  ...
  bot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('remove_mirror_')) {
      try {
        ...
        // Delete copy-trade
        await copyTradeService.deleteCopyTrade(mirrorId);
    ...
    if (data.startsWith('pause_mirror_') || data.startsWith('resume_mirror_')) {
      try {
        ...
        // Update copy-trade
        await copyTradeService.toggleCopyTradeActive(mirrorId, active);
    ...
    if (data.startsWith('toggle_sandbox_')) {
      try {
        ...
        await copyTradeService.updateCopyTrade(mirrorId, { sandboxMode: newSandboxMode });
 ...
 */
async function updateMirrorsMessage(bot, chatId, messageId, userId, userCopyTrades) {
  try {
    if (userCopyTrades.length === 0) {
      const options = {
        reply_markup: {
