 ...
 */
function configureAlerts(userId, config): any {
 ...
 */
function addToWatchlist(userId, tokenAddress): any {
 ...
 */
function removeFromWatchlist(userId, tokenAddress): any {
  if (userAlerts.has(userId)) {
    const alerts = userAlerts.get(userId);
 ...
 */
function checkPriceAlert(tokenAddress, oldPrice, newPrice): any {
 ...
 */
function checkLiquidityAlert(tokenAddress, oldLiquidity, newLiquidity): any {
 ...
 */
function checkWhaleAlert(tokenAddress, amountUSD, txHash): any {
  for (const [userId, alerts] of userAlerts.entries()) {
 ...
 */
function sendContentAlert(creator, action, details): any {
  for (const [userId, alerts] of userAlerts.entries()) {
