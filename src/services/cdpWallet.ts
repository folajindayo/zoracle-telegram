...
async function createWallet(userId, password, pin): Promise<any> {
  try {
    console.log(
      `Creating CDP wallet for user ${userId} (Simulation Mode: ${SIMULATION_MODE})`
    );

    // Use default values if password or PIN are empty
    ...
    if (SIMULATION_MODE) {
      ...
      try {
        ...
        console.log(
          `Created wallet with ID: ${walletId} and address: ${walletAddress}`
        );
      } catch (apiError) {
        ...
        throw new Error(
          `Failed to create wallet through API: ${apiError.message}`
        );
      }
    }
    ...
    try {
      ...
      const key = crypto.scryptSync(defaultPassword, salt, 32);
...
async function importWallet(userId, privateKey, password, pin): Promise<any> {
  ...
  if (SIMULATION_MODE) {
    ...
    try {
      ...
      console.log(
        `Created simulated wallet from private key with address: ${walletAddress}`
      );
    ...
    return {
      success: false,
      message:
        "Direct private key import is not supported with UseZoracle API. Please use createWallet instead.",
    };
  }
}
    ...
    if (SIMULATION_MODE) {
      ...
      try {
        ...
        if (apiError.response && apiError.response.status === 429) {
          ...
          if (localAddress) {
            console.log(
              `Rate limited - falling back to local wallet address: ${localAddress}`
            );
            return localAddress;
          }
          return null;
...
async function getTokenBalance(userId, tokenAddress): Promise<any> {
  try {
    ...
    if (SIMULATION_MODE) {
      ...
      try {
        console.log(
          `Getting token balance from UseZoracle API for token: ${tokenAddress}`
        );
        ...
        const tokenData = response.data.data.balances.find(
          (item) =>
            item.token &&
...
async function transferTokens(
  ...
  try {
    ...
    if (SIMULATION_MODE) {
      ...
      console.log(
        `Simulating transfer of ${amount} ${token} to ${toAddress} on network ${network}`
      );
      ...
      try {
        console.log(
          `Initiating transfer via UseZoracle API: ${amount} ${token} to ${toAddress}`
        );
        ...
        if (!response.data || !response.data.success || !response.data.data) {
          throw new Error(
            "Failed to initiate transfer: Invalid response format"
          );
        }
