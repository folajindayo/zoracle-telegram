...
async function createWallet(userId, password, pin): Promise<any> {
  try {
    ...
    const walletInfo = await encryptAndSaveWallet(
      userId,
      wallet.privateKey,
      password,
      pin
...
async function get2FAQRCode(userId): Promise<any> {
  try {
    ...
    // Generate QR code
    const otpauth_url = `otpauth://totp/Zoracle:${walletData.address.substring(
      0,
      8
    )}?secret=${walletData.twoFactorSecret}&issuer=Zoracle`;
