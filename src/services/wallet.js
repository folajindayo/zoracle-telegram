 ...
 */
async function quickUnlockWallet(userId, pin) {
  try {
    // Check if wallet file exists
    const walletFile = path.join(WALLETS_DIR, `${userId}.json`);
    if (!fs.existsSync(walletFile)) {
 ...
 */
function getFailedAttempts(userId) {
 ...
 */
async function getTokenBalance(userId, tokenAddress) {
  try {
    // Get wallet address
    const address = getWalletAddress(userId);
    if (!address) {
