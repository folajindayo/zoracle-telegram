...
const { formatEther, parseEther, formatUnits, parseUnits } = require('ethers/lib/utils');
 ...
 */
async function getTokenPrice(tokenAddress, outputTokenAddress = CONFIG.TOKENS.WETH, amount = '1') {
  try {
    // Get token decimals
 ...
 */
async function isTokenApproved(tokenAddress, walletAddress, spender = CONFIG.DEX_ROUTERS.AERODROME) {
  try {
    const tokenContract = new Contract(tokenAddress, ABIS.ERC20_ABI, provider);
 ...
 */
async function approveToken(tokenAddress, privateKey, spender = CONFIG.DEX_ROUTERS.AERODROME) {
  try {
 ...
 * @returns {Promise<Object>} - Transaction receipt
 */
async function buyTokensWithEth(tokenAddress, amountETH, privateKey, slippage = CONFIG.DEFAULT_SLIPPAGE, telegramId) {
  let pendingTx;
  try {
    // Create wallet from private key
    const wallet = new Wallet(privateKey, provider);
 ...
 * @returns {Promise<Object>} - Transaction receipt
 */
async function sellTokensForEth(tokenAddress, amount, privateKey, slippage = CONFIG.DEFAULT_SLIPPAGE, telegramId) {
  let pendingTx;
  try {
    // Create wallet from private key
    const wallet = new Wallet(privateKey, provider);
 ...
 */
async function splitBuyOrder(tokenAddress, amountETH, privateKey, telegramId) {
 ...
 * @returns {Promise<Array>} - Array of transaction receipts
 */
async function splitSellOrder(tokenAddress, amount, privateKey, telegramId) {
