...
module.exports = (bot, users) => {
  ...
  bot.onText(/\/trending/, async (msg) => {
    ...
    try {
      ...
      // Build message
      let message = '🔥 <b>Trending Tokens</b>\n\n';
      
      for (const token of trendingTokens) {
        message += `<b>${token.name} (${token.symbol})</b>\nCreator: ${token.creator}\nPrice: ${token.price} ETH\nVolume 24h: ${(token as any).volume24h} ETH\nLiquidity: ${token.liquidity} ETH\n\n`;
      }
...
Price: ${token.price} ETH
Liquidity: ${token.liquidity} ETH
${(token as any).volume24h ? `Volume 24h: ${(token as any).volume24h} ETH` : ''}

<a href="https://basescan.org/token/${token.address}">View on Basescan</a>
        `;
