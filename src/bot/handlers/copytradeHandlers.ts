...
module.exports = (bot, users) => {
  ...
  copyTradeEvents.on("mirror_trade_executed", async (data) => {
    const { telegramId, type, tokenAddress, ethAmount, tokenAmount, txHash } =
      data;
  ...
  bot.onText(/\/mirror (.+)/, async (msg, match) => {
    ...
    bot.sendMessage(
      chatId,
      "Please enter the maximum slippage percentage (1-10):"
    );
  });
  ...
  bot.onText(/\/mirrors/, async (msg) => {
    ...
    try {
      ...
      if (userCopyTrades.length === 0) {
        ...
        bot.sendMessage(
          chatId,
          "You don't have any active mirrors. Use /mirror to set up a mirror.",
          options
        );
        return;
      }
      ...
      bot.sendMessage(
        chatId,
        `❌ Error fetching your mirrors: ${error.message}`
      );
    }
  });
      ...
      switch (copyTradeState.state) {
        ...
        case COPYTRADE_STATES.AWAITING_SLIPPAGE: {
          ...
          if (isNaN(slippage) || slippage < 0.1 || slippage > 10) {
            bot.sendMessage(
              chatId,
              "❌ Invalid slippage. Please enter a number between 0.1 and 10."
            );
            return;
          }
        ...
        case COPYTRADE_STATES.AWAITING_MAX_ETH: {
          // Validate max ETH
          const maxEthPerTrade = parseFloat(text);
          ...
          const options = {
            reply_markup: {
              inline_keyboard: [
                [
                  {
                    text: "🧪 Sandbox Mode (Simulated)",
                    callback_data: "sandbox_yes",
                  },
          ...
          bot.sendMessage(
            chatId,
            "Do you want to enable sandbox mode? In sandbox mode, trades will be simulated and no real transactions will be made.",
            options
          );
          break;
        }
  ...
  bot.on("callback_query", async (query) => {
    ...
    if (data === "sandbox_yes" || data === "sandbox_no") {
      ...
      if (!copyTradeState) {
        bot.answerCallbackQuery(query.id, {
          text: "❌ Copy-trade setup expired. Please start again.",
        });
        return;
      }

      // Update state and auto-confirm
      const sandboxMode = data === "sandbox_yes";

      // Create copy-trade in database directly
      try {
        ...
        bot.sendMessage(
          chatId,
          `❌ Error setting up copy-trade: ${error.message}`
        );
        copyTradeStates.delete(userId);
      }
    ...
    if (data === "remove_mirror") {
      try {
        ...
        if (userCopyTrades.length === 0) {
          bot.answerCallbackQuery(query.id, {
            text: "❌ You don't have any mirrors to remove.",
          });
          return;
        }
        ...
        // Add cancel button
        inlineKeyboard.push([
          { text: "❌ Cancel", callback_data: "cancel_mirror_action" },
        ]);
    ...
    if (data.startsWith("remove_mirror_")) {
      try {
        ...
        // Acknowledge callback
        bot.answerCallbackQuery(query.id, {
          text: "✅ Mirror removed successfully!",
        });

        // Update mirrors message
        const userCopyTrades = await CopyTradeOps.getUserCopyTrades(userId);
        updateMirrorsMessage(
          bot,
          chatId,
    ...
    if (data === "pause_mirror" || data === "resume_mirror") {
      try {
        ...
        const relevantCopyTrades = userCopyTrades.filter(
          (ct) => ct.active !== active
        );

        if (relevantCopyTrades.length === 0) {
          bot.answerCallbackQuery(query.id, {
            text: `❌ You don't have any mirrors to ${actionText}.`,
          });
          return;
        }
    ...
    if (data.startsWith("pause_mirror_") || data.startsWith("resume_mirror_")) {
      try {
        ...
        const mirrorId = data.replace(
          `${active ? "resume" : "pause"}_mirror_`,
          ""
        );
        ...
        // Acknowledge callback
        bot.answerCallbackQuery(query.id, {
          text: `✅ Mirror ${actionText} successfully!`,
        });
    ...
    if (data === "toggle_sandbox") {
      try {
        ...
        if (userCopyTrades.length === 0) {
          bot.answerCallbackQuery(query.id, {
            text: "❌ You don't have any mirrors to modify.",
          });
          return;
        }
    ...
    if (data.startsWith("toggle_sandbox_")) {
      try {
        ...
        await (copyTradeService as any).updateCopyTrade(mirrorId, {
        ...
        bot.answerCallbackQuery(query.id, {
          text: `✅ Mirror set to ${modeText} mode!`,
        });
 ...
 */
async function updateMirrorsMessage(
  bot,
  chatId,
  messageId,
  userId,
